defmodule RealtimeWeb.JwtVerificationRS256Test do
  ...
  test "verify/1 when token is not a string" do
    assert {:error, :not_a_string} = JwtVerification.verify([], @jwt_secret, @alg, @pubkey)
  end
  ...
  test "verify/1 when token has invalid format" do
    ...
    assert {:error, :expected_claims_map} =
             JwtVerification.verify(invalid_token, @jwt_secret, @alg, @pubkey)
  end
  ...
  test "verify/1 when token is valid and alg is RS256" do
    ...
    assert {:ok, _claims} = JwtVerification.verify(token, @jwt_secret, @alg, @pubkey)
  end
  ...
  test "verify/1 when token has expired" do
    ...
    assert {:error, [message: "Invalid token", claim: "exp", claim_val: 1_610_086_801]} =
             JwtVerification.verify(token, @jwt_secret, @alg, @pubkey)
