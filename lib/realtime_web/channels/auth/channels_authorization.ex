defmodule RealtimeWeb.ChannelsAuthorization do
  ...
  def authorize(token, secret, signing_method, pubkey) when is_binary(token) do
    token
    |> clean_token()
    |> RealtimeWeb.JwtVerification.verify(secret, signing_method, pubkey)
  end

  def authorize(_token, _secret, _signing_method, _pubkey), do: :error

  defp clean_token(token) do
    Regex.replace(~r/\s|\n/, URI.decode(token), "")
  end

  def authorize_conn(token, secret, signing_method, pubkey) do
    case authorize(token, secret, signing_method, pubkey) do
      {:ok, claims} ->
